module.exports = {
    ...
    create_parking_lot: function (nLots) {
        var parkingCapacity = parseInt(nLots.split(" ")[1]);
        if (isNaN(parkingCapacity)) {
            return false;
        }
        else {
    ...
    leave: function (parkingCapacity, parkingLotArray, input) {
        if (parkingCapacity > 0) {
            ...
            if (parkingLotArray[index][index] === undefined || parkingLotArray[index][index] === null) {
                return -1;
            }
